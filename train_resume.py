...
if __name__ == '__main__':
     ...
     dataset= os.path.basename(os.path.normpath(args.test_dir))
    ...
    # Load score model
     ...
     model.add_para(args.N_min, args.N_max, args.t_eps_min, args.t_eps_max, 
                    args.batch_size, args.inference_N, mid_stop)
     ...
     if args.no_wandb:
          ...
          if model.ode.__class__.__name__ == "OTFLOW":
               ...
               logger = WandbLogger(project=f"{model.ode.__class__.__name__}_FINETUNING", log_model=True, save_dir="logs", name=f"{model.ode.__class__.__name__}_N_min_{args.N_min}_N_max_{args.N_max}_dataset_{dataset}_mid_stop_{mid_stop}")
     ...
     callbacks = [ModelCheckpoint(dirpath=f"logs/{model.ode.__class__.__name__}_N_min_{args.N_min}_N_max_{args.N_max}_dataset_{dataset}_mid_stop_{mid_stop}_{logger.version}", save_last=True, filename='{epoch}-last')]
     checkpoint_callback_last = ModelCheckpoint(dirpath=f"logs/{model.ode.__class__.__name__}_N_min_{args.N_min}_N_max_{args.N_max}_dataset_{dataset}_mid_stop_{mid_stop}_{logger.version}",
          save_last=True, filename='{epoch}-last')
     checkpoint_callback_pesq = ModelCheckpoint(dirpath=f"logs/{model.ode.__class__.__name__}_N_min_{args.N_min}_N_max_{args.N_max}_dataset_{dataset}_mid_stop_{mid_stop}_{logger.version}", 
          save_top_k=2, monitor="pesq", mode="max", filename='{epoch}-{pesq:.2f}')
     checkpoint_callback_si_sdr = ModelCheckpoint(dirpath=f"logs/{model.ode.__class__.__name__}_N_min_{args.N_min}_N_max_{args.N_max}_dataset_{dataset}_mid_stop_{mid_stop}_{logger.version}", 
